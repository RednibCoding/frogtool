...
supported_rom_ext = [
    "bkp", "zip", "zfc", "zsf", "zmd", "zgb", "zfb", "zpc", "smc", "fig", "sfc", "gd3", "gd7", "dx2", "bsx", "swc",
    "nes", "nfc", "fds", "unf", "gba", "agb", "gbz", "gbc", "gb", "sgb", "bin", "md", "smd", "gen", "sms", "pce"
]
zxx_ext = {
    "ARCADE": "zfb", "FC": "zfc", "GB": "zgb", "GBA": "zgb", "GBC": "zgb", "MD": "zmd", "SFC": "zsf", "PCE": "zpc"
}
supported_img_ext = [
    "png", "jpg", "jpeg", "gif"
]
...
def process_sys(drive, system, test_mode, platform):
    ...
    if system != "ARCADE":
        convert_zip_image_pairs_to_zxx(roms_path, system, platform)
...
def convert_zip_image_pairs_to_zxx(roms_path, system, platform):
    ...
    for img_file in img_files:
        ...
        if not zip_file:
            continue
        converted = convert_zip_image_to_zxx(roms_path, img_file, zip_file, sys_zxx_ext, platform)
        if not converted:
...
def convert_zip_image_to_zxx(path, img_file, zip_file, zxx_ext, platform):
    ...
    if platform == "8BK":
        converted = bgra8888_convert(img_file_path, zxx_file_path, (144, 208))
    else:
...
def bgra8888_convert(src_filename, dest_filename, dest_size=None):
    try:
        srcimage = Image.open(src_filename)
    except (OSError, IOError):
        print(f"! Failed opening image file {src_filename} for conversion")
    ...
    # Resize the image if a destination size is specified
    if dest_size:
        srcimage = srcimage.resize(dest_size)
    ...
    # Convert the image to RGBA if it was not already
    srcimage = srcimage.convert('RGBA')
    image_width, image_height = srcimage.size
    pixels = srcimage.load()

    for y in range(image_height):
        for x in range(image_width):
            r, g, b, a = pixels[x, y]
            # Write out the pixel data in ARGB order
            dest_file.write(struct.pack('BBBB', b, g, r, a))
...
def check_plat_valid(platform):
    return platform in platforms

def run():
    global systems, platforms
    print("frogtool v0.3.0")
    ...
    platform = sys.argv[3].upper() if len(sys.argv) >= 4 and sys.argv[3] not in flags else ""
    ...
    if not platform and drive:  # default to SF2000 if not specified at command line, for backwards compatibility
        platform = "SF2000"

    while not platform or not check_plat_valid(platform):
        if platform and not check_plat_valid(platform):
            print("! Specified platform is not one of the accepted options")
        platform_list = ', '.join(sorted(platforms))
        print()
        print(f"Please enter the platform to build for: {platform_list}")
    ...
    while not drive or not os.path.isdir(drive):
        ...
        print()
        print(f"Please enter the drive or path where your {platform} SD card is located e.g. F:")
        drive = input()

    systems = platforms.get(platform)
    ...
    while not system or not check_sys_valid(system):
        ...
        systems_list = ', '.join(sorted(systems.keys()))
        print()
        print(f"Please enter the system to rebuild game list for: {systems_list} or ALL")
        system = input().upper()
    ...
    try:
        ...
        for syskey in keys_to_process:
            process_sys(drive, syskey, test_mode, platform)
